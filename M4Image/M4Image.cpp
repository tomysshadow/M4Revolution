...
typedef std::map<M4Image::FORMAT, pixman_format_code_t> PIXMAN_FORMAT_CODE_MAP;
...
static const PIXMAN_FORMAT_CODE_MAP RGBA_PIXMAN_FORMAT_CODE_MAP = {
    {M4Image::FORMAT::RGB, PIXMAN_r8g8b8},
    {M4Image::FORMAT::RGBA, PIXMAN_a8r8g8b8},
    {M4Image::FORMAT::RGBX, PIXMAN_x8r8g8b8},
    {M4Image::FORMAT::BGR, PIXMAN_b8g8r8},
    {M4Image::FORMAT::BGRA, PIXMAN_a8b8g8r8},
    {M4Image::FORMAT::BGRX, PIXMAN_x8b8g8r8},
    {M4Image::FORMAT::A, PIXMAN_a8}
};

static const PIXMAN_FORMAT_CODE_MAP BGRA_PIXMAN_FORMAT_CODE_MAP = {
    {M4Image::FORMAT::RGB, PIXMAN_b8g8r8},
    {M4Image::FORMAT::RGBA, PIXMAN_a8b8g8r8},
    {M4Image::FORMAT::RGBX, PIXMAN_x8b8g8r8},
    {M4Image::FORMAT::BGR, PIXMAN_r8g8b8},
    {M4Image::FORMAT::BGRA, PIXMAN_a8r8g8b8},
    {M4Image::FORMAT::BGRX, PIXMAN_x8r8g8b8},
    {M4Image::FORMAT::A, PIXMAN_a8}
};

// the FORMAT enum uses little endian, like mango
...
M4Image::FORMAT getResizeFormat(
    ...
    switch (format) {
        ...
        break;
        case M4Image::FORMAT::L:
        // here, the image is loaded in XXXL format
        ...
        return M4Image::FORMAT::XXXL;
        case M4Image::FORMAT::XXXL:
        // here we just keep it 32-bit the whole way, easy peasy
        // we do the same trick as FORMAT::L where luminance = Pixman's alpha
        sourceFormat = PIXMAN_a8r8g8b8;
        destinationFormat = PIXMAN_a8r8g8b8;
        return M4Image::FORMAT::XXXL;
        case M4Image::FORMAT::AL:
        case M4Image::FORMAT::XXLA:
        // for FORMAT::AL, mango reads the image in XXLA format
    ...
    if (imageHeader.format == IMAGE_HEADER_FORMAT_RGBA) {
        ...
        destinationFormat = RGBA_PIXMAN_FORMAT_CODE_MAP.at(format);
        return M4Image::FORMAT::RGBA;
    }
    ...
    destinationFormat = BGRA_PIXMAN_FORMAT_CODE_MAP.at(format);
    return M4Image::FORMAT::BGRA;
}
...
unsigned char* M4Image::load(
    ...
    setFormat(surface.format, format);
...
unsigned char* M4Image::resize(
    ...
    bool convert = format == M4Image::FORMAT::AL;

    pixman_format_code_t sourceFormat = PIXMAN_x8r8g8b8;
    pixman_format_code_t destinationFormat = PIXMAN_a8r8g8b8;

    if (resize) {
        format = getResizeFormat(imageHeader, format, sourceFormat, destinationFormat);
    }
    ...
    M4Image::Color32* colorPointer = (M4Image::Color32*)bits;
    M4Image::Color32* endPointer = (M4Image::Color32*)(bits + bitsSize);
...
void M4Image::setAllocator(MAllocProc _mallocProc, FreeProc _freeProc) {
