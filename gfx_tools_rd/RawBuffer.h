...
namespace gfx_tools {
	struct RawBuffer {
		...
		GFX_TOOLS_RD_API RawBuffer(POINTER pointer, SIZE size, bool owner = false);
		GFX_TOOLS_RD_API ~RawBuffer();
	...
	struct RawBufferEx : public RawBuffer {
		...
		GFX_TOOLS_RD_API RawBufferEx(POINTER pointer, SIZE size, bool owner = false, const std::optional<ResizeInfo> &resizeInfoOptional = std::nullopt);
	};
}